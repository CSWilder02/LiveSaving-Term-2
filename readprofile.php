...
while ($row = $result->fetch_assoc()) {
    if ($row['id'] == $_GET['id']) {
        ...
        echo '<form class="form-inline m-2" action="updateprofile.php" method="POST">';
        ...
        echo '<div class="form-group">';
        echo '<label for="date">Date:</label>';
        echo '<input type="text" class="form-control" name="rank" value="' . $row['rank'] . '">';
        echo '</div>';
        ...
        echo '<div class="card-body">';
        echo '<p class="card-text">Name and Surname: ' . $row['name_surname'] . '</p>';
        ...
        echo '<a class="btn btn-primary" style="background-color: #0B466C; margin-top: 30px; margin-right:20px; width: 300px; padding-top: 10px; padding-bottom: 10px" href="receprofile.php?id=' . $row['id'] . '">Update</a>';
        echo '<a class="btn btn-danger" style="width: 90px; margin-top: 30px; background-color: rgb(97, 0, 0);" href="deleteprofile.php?id=' . $row['id'] . '"><img style="width: 25px" src="Images\recycle-bin (1).png" alt=""></a>';
        echo '</div>';
        echo '</div>';
    }
}
